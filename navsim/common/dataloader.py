...
def filter_scenes(
    data_path: Path, sensor_blobs_path: Path, scene_filter: SceneFilter
    ...
    :param sensor_blobs_path: root directory of sensor data to check for file existence
    ...
    for log_pickle_path in tqdm(log_files, desc="Loading logs"):
        ...
        for frame_list in split_list(
            ...
            # Filter by token
            frame_dict = frame_list[scene_filter.num_history_frames - 1]
            token = frame_dict["token"]
            if filter_tokens and token not in tokens:
                continue

            # --- New Logic: Check for file existence before including the scene ---
            # Check for Lidar file
            lidar_file_path = Path(sensor_blobs_path) / frame_dict["lidar_path"]
            if not lidar_file_path.exists():
                # Lidar file is missing, skip this scene
                continue

            # Check for all camera files
            all_cams_exist = True
            for cam_data in frame_dict["cams"].values():
                cam_file_path = Path(sensor_blobs_path) / cam_data["data_path"]
                if not cam_file_path.exists():
                    all_cams_exist = False
                    break

            if not all_cams_exist:
                # One or more camera files are missing, skip this scene
                continue
...
class SceneLoader:
    ...
    def __init__(
        ...
        self.scene_frames_dicts = filter_scenes(
            data_path, sensor_blobs_path, scene_filter
        )
